...
function Sidebar() {
    ...
    return (
        <div className={`md:sticky top-0 left-0 h-screen w-0 duration-500 ${open ? "sm:w-[104px]": "sm:w-[300px]"}`}>
            <button className={`md:absolute fixed top-5 md:right-4 md:left-auto left-5 flex flex-col justify-center items-center md:bg-slate-100/[0.4] bg-white backdrop-blur-md gap-1 w-5 h-6 z-[50] p-[2px] px-[13px] rounded-full`} onClick={() => setOpen(!open)}>
                { open ?
