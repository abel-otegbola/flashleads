...
export const AuthCTA = ({ user }: { user: { email?: string } | null }) => {
    if (user) {
        return (
            <Link to={"/account"} className="w-10 h-10 rounded-full bg-primary/[0.2] aspect-square outline outline-offset-2 outline-gray-200/[0.2] border border-gray/[0.2] flex items-center justify-center font-semibold">
